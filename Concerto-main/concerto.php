...
public static function Sala($id) //metodo per la dircerca di un record tramite il suo id : verrà ritornato il corrispondente record
{
    $db = new dbManager("config.txt"); //classe utilizzata per gestire il database organizzazione_concerti
    $db->connessione(); //metodo per la connessione al database tramite PDO

    if ($fetch = $db->findsala($id)) { //metodo per la ricerca dell'oggetto tramite id all'interno della tabella concerti
...
function sala()
{
    ...
    if ($sala = Concerto::Sala($id)) {
        echo "Informazioni della sala:\n";
